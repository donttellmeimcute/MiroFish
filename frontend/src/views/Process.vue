      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">The graph build will start automatically once ontology generation is complete.</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">Graph build in progress</p>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After the documents are uploaded, the LLM analyzes their content and automatically generates an ontology suitable for public-opinion simulation (entity types + relationship types).
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  Using the generated ontology, the system chunks the documents and calls the Zep API to build a knowledge graph that extracts entities and relationships.
                </div>
              </div>
...
const currentPhase = ref(-1) // -1: 上传中, 0: Ontology Generation中, 1: Graph Building, 2: 完成
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Graph build in progress'
...
const handleNewProject = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'Ontology Generation失败'
    }
  } catch (err) {
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || '启动Graph Building失败'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = '启动Graph Building失败: ' + (err.message || '未知错误')
    buildProgress.value = null
  }
}
...
const fetchGraphData = async () => {
  try {
    // 先获取Project Information以获取 graph_id
