...
const handleNextStep = (params = {}) => {
  ...
  if (params.maxRounds) {
    ...
    addLog('Using the automatically generated round count')
  }
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('A running simulation environment was detected. Closing it now...')
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Simulation status is running. Stopping it now...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
